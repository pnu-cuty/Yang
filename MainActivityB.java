        ...
        builder.setPositiveButton("확인", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (TextUtils.isEmpty(id)) {
                    Toast.makeText(MainActivityB.this, "ID를 입력해주세요.", Toast.LENGTH_SHORT).show();
                } else SearchID(id);
            }
        });
    ...
    public void SearchID(final String id) {
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    if (snapshot.child("ID").getValue().toString().equals(id)) {
                        ...
                        if(snapshot.child("FLAG").getValue().toString().equals("S")) {
                            Intent intent = new Intent(MainActivityB.this, mypage_seller.class);
                            intent.putExtra("ID", id);
                            startActivity(intent);
                        }
                        else {
                            Intent intent = new Intent(MainActivityB.this, mypage_buyer.class);
                            intent.putExtra("ID", id);
                            startActivity(intent);
                        }
                    }
                ...
                if (!isSearched)
                    Toast.makeText(getApplicationContext(), "검색하신 ID가 존재하지 않습니다.", Toast.LENGTH_SHORT).show();
            }
